# go-ollama

An [ollama](https://github.com/ollama/ollama) API library for Go.

## Usage

### Install

To add go-ollama to your project, run:
```shell
go get github.com/JexSrs/go-ollama
```

### Ollama instance

```go
llm := ollama.New("http://localhost:11434")
```

To set the default model use:
```go
llm.SetDefaultModel("llama3")
```

To append extra headers in the request:
```go
llm.WithHeader("key", {"value"})
```

### Generate a completion

```go
res, err := llm.Generate(
    llm.Generate.WithModel("llama3"), // Default value
    llm.Generate.WithPrompt("Why is the sky blue?"),
)
```

By default, stream is set to `false`, to enable it:
```go
res, err := llm.Generate(
    llm.Generate.WithModel("llama3"), // Default value
    llm.Generate.WithPrompt("Why is the sky blue?"),
	llm.Generate.WithStream(true, 1024, func(r *Response, err error) {
        // ...		
    }),
)
```

The function will block the thread until streaming is finished and return the latest response
with the concatenated message of the previous responses.

To apply a format in the model's response:
```go
res, err := llm.Generate(
    llm.Generate.WithModel("llama3"), // Default value
    llm.Generate.WithPrompt("What color is the sky at different times of the day? Respond using JSON"), // Important to instruct the model to respond in json
    llm.Generate.WitFormat("json"),
)
```

To append am image to the request:
```go
res, err := llm.Generate(
    llm.Generate.WithModel("llama3"), // Default value
    llm.Generate.WithPrompt("What is in this picture?"),
    llm.Generate.WithImage(""),
)
```

To set temperature and seed:
```go
res, err := llm.Generate(
    llm.Generate.WithModel("llama3"), // Default value
    llm.Generate.WithPrompt("Hello, how are you?"),
    llm.Generate.WithTemperature(0.4),
    llm.Generate.WithSeed(123),
)
```

### Generate a chat completion

```go
chatid := "gyhztyd"
message := Message {
    Role: Pointer("user"),
	Content: Pointer("Hello, how are you?"),
	Images: nil,
}

res, err := llm.Chat(
	chatId,
    llm.Chat.WithModel("llama3"), // Default value
    llm.Chat.WithMessage(message),
)
```

By passing a chat id, the request will include all the previous messages sent and received before the current message.
If the chatId does not exist, it will create a new one.
If you don't want to keep history then pass `nil` as the `chatId`.

> All chats are stored into memory. A restart of the application will delete all chats

To handle the chats:
```go
chat := ll.GetChat(chatId) // Returns the chat or nil if it does not exist
llm.DeleteChat(chatId) // Deletes a specified chat
llm.DeleteAllChats() // Delete all chats

llm.PreloadChat(chat) // Loads a new Chat instance
```

To access the chats:
```go
chat := ll.GetChat(chatId)

chat.AddMessage(Message{...}) // Adds a new message at the end of the list
chat.AddMessageTo(2, Message{...}) // Adds a new message at the specified index
chat.DeleteMessage(2) // Deletes a message at the specified index
chat.DeleteAllMessages() // Deletes all mesages
```

### Rest functions

To create a blob:
```go
success, err := llm.BlobCreate("sha256:...", []byte{...})
```

Check blob exists:
```go
success, err := llm.BlobCheck("sha256:...")
```

Create a model:
```go
res, err := llm.CreateModel(
    llm.CreateModel.WithFrom("llama3"),
    llm.CreateModel.WithParameter(Parameter{Key:"num_keep", Value:"24"),
    llm.CreateModel.WithStream(true, 1024, func(r *Response, err error) {
        // ...		
    }),
	llm.CreateModel.WithLicense("<license>"),
)
```

Get local models:
```go
res, err := llm.ListLocalModels()
```

Show model's information:
```go
res, err := llm.ShowModelInfo("llama3")
```

Clone a model:
```go
res, err := llm.CopyModel("llama3", "llama3-copy")
```

Delete a model:
```go
res, err := llm.DeleteModel("llama3")
```

Pull a model from ollama library:
```go
res, err := llm.PullModel(
    llm.PullModel.WithName("llama3"),
    llm.PullModel.WithInsecure(false),
    llm.PullModel.WithStream(true, 1024, func(r *Response, err error) {
        // ...		
    }),
)
```

Push a model to the ollama library:
```go
res, err := llm.PushModel(
    llm.PushModel.WithName("llama3"),
    llm.PushModel.WithInsecure(false),
    llm.PushModel.WithStream(true, 1024, func(r *Response, err error) {
        // ...		
    }),
)
```

Generate embeddings:
```go
res, err := llm.GenerateEmbeddings(
    llm.GenerateEmbeddings.WithModel("llama3"),
    llm.GenerateEmbeddings.WithPrompt("This is the prompt!"),
    llm.GenerateEmbeddings.WithKeepAlive("5m"),
    llm.GenerateEmbeddings.WithStream(true, 1024, func(r *Response, err error) {
        // ...		
    }),
)
```

